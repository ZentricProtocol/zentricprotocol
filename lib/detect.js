...
const STRUCTURAL_SIGNATURES = [
  ...
  {
    // Unusual delimiter stacking used to forge a context break: long runs of
    // delimiter characters, code-fence stacks, or angle/pipe walls.
  ...
  {
    // Requests to send/output data to an external URL or endpoint.
    sig: 'DATA_EXFILTRATION',
    regex: /\b(?:send|post|upload|exfiltrate|transmit|forward|leak|deliver|email|curl|wget|fetch)\b[^.?!\n]{0,50}(?:https?:\/\/|ftp:\/\/|www\.|\b(?:to\s+)?(?:the\s+)?(?:url|endpoint|server|webhook|address|api)\b)/i,
  },
];
...
export function analyze(input, modules = ['integrity', 'privacy']) {
  ...
  // earlier offsets stay valid). Redacts ALL occurrences.
  let anonymizedInput = input;
  if (piiDetected) {
    for (const e of [...entities].sort((a, b) => b.start - a.start)) {
      anonymizedInput = anonymizedInput.slice(0, e.start) + '[REDACTED]' + anonymizedInput.slice(e.end);
    }
  }
